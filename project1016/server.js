...
//app.use(express.json());//파라미터를 json 형식으로 받고싶을떄 사용할 미들웨어
...
app.post("/notice/regist",function(request, response){
    ...
    con.query(sql,[
        ...
        Writer,
        content
    ],function(error,record,field){
        if(error){
            console.log(error);
        }else{
            response.writeHead(200,{"Content-Type":"text/html;charset=utf-8"});
            response.end(common.getMsgURL("성공!","/notice/list"));
        }
    })
...
//목록 가져오기
    ...
    app.post("/notice/del",function(request,response){
        ...
        con.query(sql,function(error,record,field){
            if(error){
                ...
                response.writeHead(200,{"Content-Type":"text/html;charset=utf-8"});
                response.end(common.getMsgURL("삭제되었습니다","/notice/list"));
            }
        })
    ...
    app.post("/notice/edit",function(request,response){
        ...
        con.query(sql,[title,Writer,content,notice_id],function(error,record,field){
            if(error){
                console.log(error);
            }else{
                response.writeHead(200,{"Content-Type":"text/html;charset=utf-8"});
                response.end(common.getMsgURL("수정되었습니다","/notice/list"));
            }
        })
    })
